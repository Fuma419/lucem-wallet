---
name: commit-scope-discipline
description: >-
  Keep commits scoped, verify lightly before push, and ship via PR until
  Jenkins merges. Use when staging, committing, or pushing Lucem Wallet work.
---
...
4. Never commit: `build/`, `secrets.*.js` (except `secrets.testing.js`), `node_modules/`, `.vercel/`, `.env`.
5. **Before push:** optionally run `NODE_ENV=test npx jest` for fast feedback. Do **not** run local webpack/`npm run build` — Jenkins owns Build / Unit / Integration / Functional gates. If you change CSS/UI strings that unit tests assert, update those tests in the **same** commit.
