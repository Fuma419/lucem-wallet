...
describe('hw.jsx mobile layout and Ledger Web Bluetooth', () => {
  ...
  test('Ledger USB is offered next to Bluetooth on the hardware tab', () => {
    ...
    expect(transportSrc).toContain('openConnected');
    ...
    expect(hwSrc).toContain('formatLedgerError');
  ...
  test('Keystone confirm shows the send review before opening the QR tab', () => {
    ...
    expect(modalSrc).toContain('pickLedgerUsbDevice');
    expect(modalSrc).toContain('pickLedgerBluetoothDevice');
    expect(modalSrc).toContain('flushSync');
    expect(modalSrc).toContain('listGrantedLedgerUsbPicks');
