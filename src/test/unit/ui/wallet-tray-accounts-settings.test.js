...
describe('wallet tray accounts vs settings FABs', () => {
  ...
  test('accounts is a full-screen page with account actions', () => {
    ...
    // HardwareWalletModal must expose Continue → openFlowWindow(TAB.hw). The
  ...
  test('tray destination pages drop their own top back arrows', () => {
    ...
    expect(settingsSrc).toContain('glowEffectsStored');
    expect(settingsSrc).toContain('resolveGlowEffects');
    expect(settingsSrc).toContain('Light mode is off by default');
